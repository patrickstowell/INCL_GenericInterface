...
export INCL_DATA=$INCL_BIN_DIR/data/

export INCL_SRC_INC_DIRS=$(find $INCL_SRC -type f \( -name "*.hh" -o -name "*.h" \) -exec dirname {} \; | sort -u | sed 's/^/-I/' | tr '\n' ' ')
export INCL_BUILD_INC_DIRS=$(find $INCL_BIN_DIR -type f \( -name "*.hh" -o -name "*.h" \) -exec dirname {} \; | sort -u | sed 's/^/-I/' | tr '\n' ' ')
export INCL_INC_DIRS="$INCL_SRC_INC_DIRS $INCL_BUILD_INC_DIRS"

export INCL_LIB_DIRS=$(find $INCL_BIN_DIR -type f \( -name "*.so" -o -name "*.a" \) -exec dirname {} \; | sort -u | tr '\n' ':' | sed 's/:$//')
export INCL_LIB_LINKING_DIRS=$(find $INCL_BIN_DIR -type f \( -name "*.so" -o -name "*.a" \) -exec dirname {} \; | sort -u | sed 's|^|-L|' | tr '\n' ' ')
export INCL_LIBS=$(find $INCL_BIN_DIR -type f \( -name "*.so" -o -name "*.a" \) -exec basename {} \; | sed -E 's/^lib(.*)\.(so|a)$/-l\1/' | tr '\n' ' ')
